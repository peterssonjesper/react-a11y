...
exports.props = {
  onClick: {
    ...
    BUTTON_ROLE_SPACE: {
      msg: 'You have `role="button"` but did not define an `onKeyDown` handler. Add it, and have the "Space" key do the same thing as an `onClick` handler. Or just use a <button>.',
      test (tagName, props, children) {
    ...
    BUTTON_ROLE_ENTER: {
      msg: 'You have `role="button"` but did not define an `onKeyDown` handler. Add it, and have the "Enter" key do the same thing as an `onClick` handler. Or just use a <button>.',
      test (tagName, props, children) {
        return !(props.role === 'button' && !props.onKeyDown);
      }
    }
